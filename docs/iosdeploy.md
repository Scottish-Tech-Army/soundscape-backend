...
- Deploy the function app code to the deployment. `functionapp.sh` takes the target resource group as an argument and publishes the function apps it finds there (for iOS, `trigger` and `vmcount`).
